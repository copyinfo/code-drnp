	...
	<?php foreach ( $subscriptions as $subscription_key => $subscription_details ) : ?>
		...
		<tr class="order">
			...
			<td class="subscription-next-payment">
				<?php $next_payment_timestamp = WC_Subscriptions_Order::get_next_payment_timestamp( $subscription_details['order_id'], $subscription_details['product_id'] ); ?>
				<?php if ( $next_payment_timestamp == 0 ) : ?>
					-
				<?php else : ?>
				<time title="<?php echo esc_attr( $next_payment_timestamp ); ?>">
					<?php echo date_i18n( get_option( 'date_format' ), $next_payment_timestamp ); ?>
				</time>
				<?php endif; ?>
			</td>
		</tr>
