...
class NewsletterControls {
    ...
    function feed_themes($name) {
        $themes[''] = 'Standard feed themes';
        $themes['feed-1'] = 'Feed theme 1';
        $themes['feed-2'] = 'Feed theme 2';

        $this->select_grouped($name, array(
            array_merge(array('' => 'Custom feed themes'), newsletter_feed_get_themes()),
            $themes
        ));
    }

    function followup_themes($name) {
        $themes[''] = 'Standard follow up themes';
        $themes['followup-1'] = 'Follow up theme 1';
        $themes['followup-2'] = 'Follow up theme 2';

        $this->select_grouped($name, array(
            array_merge(array('' => 'Custom follow up themes'), newsletter_followup_get_themes()),
            $themes
        ));
    }
    ...
    function textarea_fixed($name, $width='100%', $height='400px') {
...
function newsletter_search($text, $status='', $order='email', $list = null, $link = null, $limit='') {
    global $wpdb;
...
function newsletter_feed_get_themes() {
    $handle = @opendir(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-feed');
    $list = array();
    if (!$handle) return $list;
    while ($file = readdir($handle)) {
        if ($file == '.' || $file == '..') continue;
        if (!is_dir(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-feed/' . $file)) continue;
        if (!is_file(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-feed/' . $file . '/theme.php')) continue;
        $list['*' . $file] = $file;
    }
    closedir($handle);
    return $list;
...
function newsletter_followup_get_themes() {
    $handle = @opendir(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-followup');
    $list = array();
    if (!$handle) return $list;
    while ($file = readdir($handle)) {
        if ($file == '.' || $file == '..') continue;
        if (!is_dir(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-followup/' . $file)) continue;
        if (!is_file(ABSPATH . 'wp-content/plugins/newsletter-custom/themes-followup/' . $file . '/theme.php')) continue;
        $list['*' . $file] = $file;
    }
    closedir($handle);
    return $list;
