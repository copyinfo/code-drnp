...
define('NEWSLETTER_DIR', WP_PLUGIN_DIR . '/newsletter-pro');
define('NEWSLETTER_URL', WP_PLUGIN_URL . '/newsletter-pro');
define('NEWSLETTER_VERSION', '2.6.1');
...
class Newsletter {
    ...
    var $feed_excluded_categories = '';
    var $feed_posts = null;
    var $feed_max_posts = 10;
    var $feed_skip = false;
    ...
    function hook_admin_head() {
        if (strpos($_GET['page'], 'newsletter-pro/') === 0) {
          echo '<link type="text/css" rel="stylesheet" href="' . NEWSLETTER_URL . '/jquery-ui.css?' . NEWSLETTER_VERSION . '"/>';
          echo '<link type="text/css" rel="stylesheet" href="' . NEWSLETTER_URL . '/style.css?' . NEWSLETTER_VERSION . '"/>';
    ...
    /**
     * Periodically called to generate, if the right time, the email to be sent to
     * feed by mail subscribers.
     */
    function hook_newsletter_feed($force=false) {
        ...
        $total = (int) $wpdb->get_var("select count(*) from " . $wpdb->prefix . "newsletter where status='C' and feed=1");
  ...
  /**
   * New deliver function for multipart email that will be replace the send function.
   * @param type $email 
   */
  function deliver($email, $users = null) {
    global $wpdb;
    ...
    if ($users == null) {
      $query = $email->query . " and id>" . $email->last_id . " order by id limit " . $this->email_limit;
    ...
    if (empty($users)) {
      ...
      $wpdb->query("update " . NEWSLETTER_EMAILS_TABLE . " set status='sent' where id=" . $email->id);
      return true;
    }

    foreach ($users as &$user) {

      $wpdb->query("update " . NEWSLETTER_EMAILS_TABLE . " set sent=sent+1, last_id=" . $user->id . " where id=" . $email->id);

      $m = $this->replace($email->message, $user);
      $mt = $this->replace($email->message_text, $user);
      ...
      $m = str_replace('</body>', '<img src="' . plugins_url('do.php', __FILE__) . '?a=o&amp;ni=' . $user->id . '&amp;ne=' . $email->id . '"/></body>', $m);

      $s = $this->replace($email->subject, $user);

      $this->mail_multipart($user->email, $s, array('html' => $m, 'text' => $mt, true), $headers, 2);
    ...
    // That manages the new message formats
    if (strpos($email->type, 'automated') === 0 || $email->type == 'message') {
      return $this->deliver($email, $users);
    }
        ...
        foreach ($users as $user) {
            $headers = array('List-Unsubscribe' => '<' .
                $this->add_qs((empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']), 'na=u&ni=' . $user->id . '&nt=' . $user->token) . '>');
            ...
            $m = '<html><head><style type="text/css">' . $css .
                '</style></head><body>' . $m .
                '<img src="' . plugins_url('do.php', __FILE__) . '?a=o&amp;ni=' . $user->id . '&amp;ne=' . $email->id . '"/>' .
                '</body></html>';
    ...
    function feed_send($email, $users = null) {
        ...
        foreach($categories as $c) {
            if ($this->feed_options['category_' . $c->cat_ID] == 1) {
                $this->feed_excluded_categories .= '-' . $c->cat_ID . ',';
            }
        }

        $this->feed_max_posts = $this->feed_options['max_posts'];
        if (!is_numeric($this->feed_max_posts)) $this->feed_max_posts = 10;
        
        $filters = array('showposts'=>$this->feed_max_posts, 'post_status'=>'publish');
        if ($this->feed_excluded_categories != '') $filters['cat'] = $this->feed_excluded_categories;

        $this->log($filters, 3);

        $this->feed_posts = get_posts($filters);

        foreach ($users as $user) {
            $this->user = $user; // Used by newsletter theme
            $this->feed_skip = false;
...
/**
   * New mail function that will replace the internal wp_mail to be able to send
   * a multipart message. 
   * @param type $to
   * @param type $subject
   * @param array $message Array with key "html" and "text" for the two version of this email
   * @param type $headers
   * @return boolean 
   */
  function mail_multipart($to, $subject, $message, $headers = null) {
    require_once ABSPATH . WPINC . '/class-phpmailer.php';
    require_once ABSPATH . WPINC . '/class-smtp.php';
    if ($this->mailer == null)
      $this->mailer = new PHPMailer();

    // TODO: implement the SMTP management
    $this->mailer->IsMail();

    // TODO: fix it...
    $main_options = $this->options_main;

    $this->mailer->CharSet = 'UTF-8';
    $this->mailer->From = $main_options['sender_email'];
    $this->mailer->FromName = $main_options['sender_name'];

    if (!empty($main_options['content_transfer_encoding']))
      $this->mailer->Encoding = $this->options_main['content_transfer_encoding'];

    if (!empty($main_options['return_path']))
      $this->mailer->Sender = $main_options['return_path'];
    if (!empty($main_options['reply_to']))
      $this->mailer->AddReplyTo($main_options['reply_to']);
    ...
    if (!empty($message['text'])) {
      ...
      $this->mailer->AltBody = 'HTML enabled email client required.';
    }
    ...
    // DKIM
    $this->mailer->DKIM_private = NEWSLETTER_DIR . '/dkim.txt';
    $this->mailer->DKIM_domain = 'satollo.com';
    ...
    function mail($to, $subject, $message, $html=true, $headers=null, $priority=0) {
        ...
        if ($this->options_main['smtp_enabled'] == 0) {
            ...
            //if (isset($mailer) && method_exists($mailer, 'set')) $mailer->set($priority, 'newsletter');
            $h .= "MIME-Version: 1.0\n";
            ...
            //if (isset($mailer) && method_exists($mailer, 'set')) $mailer->reset();
    ...
    function relink_callback($matches) {
        ...
        if (strpos($href, 'a=') !== false && strpos($href, 'ni=') !== false && strpos($href, 'nt=') !== false) return $matches[0];

        $anchor = trim(str_replace(';', ' ', $matches[4]));
        if (!$anchor) $anchor = '-';

        $url .= plugins_url('do.php', __FILE__) . '?a=r&amp;nr=' .
    ...
    function profile_form($user) {
        ...
        $buffer .= '<form action="' . (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']) . '" method="post"><input type="hidden" name="na" value="ps"/>';
    ...
    function subscription_form($action='s') {
        ...
        $buffer = '<div class="newsletter newsletter-subscription"><form method="post" action="' . (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']) . '" onsubmit="return newsletter_check(this)"><input type="hidden" name="na" value="' . $action . '"/>';
        $buffer .= '<table>';
    ...
    function shortcode_newsletter($attrs, $content) {
        ...
        if (stripos($options['subscription_text'], '<form') !== false) {
            $buffer = str_ireplace('<form', '<form method="post" action="' . (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']) . '" onsubmit="return newsletter_check(this)"', $options['subscription_text']);
            // Adjust the lists tag
            $buffer = $this->replace_lists($buffer);
            return str_ireplace('</form>', '<input type="hidden" name="na" value="s"/></form>', $buffer);
        }

        $form = $this->form($attrs['form']);

        if (strpos($options['subscription_text'], '{subscription_form}') !== false) return str_replace('{subscription_form}', $form, $options['subscription_text']);
        else return $options['subscription_text'] . $form;
    }
    ...
    function form($number=null) {
        ...
        if (stripos($form, '<form') !== false) {
            $form = str_replace('{newsletter_url}', (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']), $form);
        }
        else {
            $form = '<form method="post" action="' . (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']) . '" onsubmit="return newsletter_check(this)">' . 
                    '<input type="hidden" name="na" value="s"/>' . $form . '</form>';
        }
    ...
    function replace_lists($buffer) {
        ...
        return str_replace('{lists}', $lists, $buffer);        
    }
    ...
    function replace($text, $user=null) {
        ...
        if ($user != null) {
            ...
            $text = $this->replace_url($text, 'UNLOCK_URL', $this->add_qs($this->options_main['lock_url'], 'na=m' . $id_token));
    ...
    function shortcode_newsletter_lock($attrs, $content=null) {
        global $hyper_cache_stop, $lite_cache_stop, $cache_stop;
        ...
        eval('?>' . $buffer . "\n");
        $buffer = ob_get_clean();
        $buffer = str_ireplace('<form', '<form method="post" action="' . (empty($this->options_main['url']) ? get_option('home') : $this->options_main['url']) . '"', $buffer);
