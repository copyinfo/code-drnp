...
class ShoppProductThemeAPI implements ShoppAPI {
	...
	static function add_to_cart ($result, $options, $O) {
		...
		if (!$ajax) {
			...
			$string .= '<div id="registerbutton"><input type="image" value="button" src="/images/register-button.png" name="addtocart" '.inputattrs($options).' /></div>';
		} else {
	...
	static function register ($result, $options, $O) {
		if (!isset($options['value'])) $options['value'] = __("Register","Shopp");
		return self::addtocart($result, $options, $O);
	}
