...
<?php
	...
	function shortcodes () {
		...
		$this->shortcodes['catalog-register'] = array(&$this,'register_shortcode');
		...
		$this->shortcodes['register'] = array(&$this,'register_shortcode');
	...
	/**
	 * Handles rendering the [product-register] shortcode
	 *
	 * @author Jonathan Davis
	 * @since 1.1
	 *
	...
	function register_shortcode ($atts) {
		...
		$properties = array('name','slug','id','option');
		foreach ($properties as $prop) {
			if (!isset($atts[$prop])) continue;
		...
		<form action="<?php shopp('cart','url'); ?>" method="post" class="shopp product">
			...
			<?php if (isset($atts['addons'])): $addons = empty($atts['addons'])?'mode=menu&label=true&defaults='.__('Select an add-on','Shopp').'&before_menu=<li>&after_menu=</li>':$atts['addons']; ?>
				...
				<?php endif; ?>
			<?php endif; ?>
			<?php echo"<input name='products[".$Product->id."][options][]' value ='".$Option."' type='hidden' />"; ?>
			<p><?php if (isset($atts['quantity'])): $quantity = (empty($atts['quantity']))?'class=selectall&input=menu':$atts['quantity']; ?>
				<?php shopp('product','quantity',$quantity); ?>
				
			<?php endif; ?>
			
			<?php $button = empty($atts['button'])?'label='.__(' ','Shopp'):$atts['button']; ?>
